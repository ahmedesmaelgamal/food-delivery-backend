...
//    }
    ...
    protected function storageLink($path, $data, $type): string|array
    {
        $fileName = is_object($data)
            ? ($data->file_name ?? $data->name ?? $data->value ?? '')
            : $data;

        if ($type === 's3' && $this->storageConnectionCheck() === 's3') {
            $fullPath = ltrim($path . '/' . $fileName, '/');
            if ($this->fileCheck(disk: 's3', path: $fullPath) && !empty($fileName)) {
                return [
                    'key' => $fileName,
                    'path' => Storage::disk('s3')->url($fullPath),
                    'status' => 200,
            ...
            if ($this->fileCheck(disk: 'public', path: $fullPath) && !empty($fileName)) {
                return [
                    'key' => $fileName,
                    'path' => dynamicStorage('storage/app/public/' . $fullPath),
                    'status' => 200,
                ];
            }
        }
